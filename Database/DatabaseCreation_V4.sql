...
/* Home View Stored Proc */

DELIMITER //
DROP PROCEDURE IF EXISTS ShowMonthlyCostForYear
// DELIMITER;

DELIMITER //
CREATE DEFINER=`dev`@`localhost` PROCEDURE `ShowMonthlyCostForYear`(
IN Year_param INT
...
BEGIN
 ...
 SELECT MONTH(Start) as Month,
		YEAR(Start) as Year,
         SUM(OperatingCost) AS TotalCost,
         SUM(OperatingRevenue - OperatingCost) AS TotalBillable,
         SUM(OperatingRevenue) AS TotalRevenue
    FROM jobs 
    INNER JOIN jobstasks ON jobs.JobId = jobstasks.JobId
    WHERE YEAR(Start) = Year_param 
...
DELIMITER //
DROP PROCEDURE IF EXISTS ShowYearlyCost
// DELIMITER;

DELIMITER //
CREATE DEFINER=`dev`@`localhost` PROCEDURE `ShowYearlyCost`(
)
BEGIN
...
DELIMITER //
DROP PROCEDURE IF EXISTS ShowMonthlyRevenue
// DELIMITER;


DELIMITER //
CREATE DEFINER=`dev`@`localhost` PROCEDURE `ShowMonthlyRevenue`(
)
BEGIN
  ...
  SELECT 
    ...
    FROM jobs 
    INNER JOIN jobstasks ON jobs.JobId = jobstasks.JobId
	WHERE MONTH(Start) = MONTH((SELECT NOW()));

END //
DELIMITER ;

DELIMITER //
DROP PROCEDURE IF EXISTS ShowMonthlyCost
// DELIMITER;


DELIMITER //
CREATE DEFINER=`dev`@`localhost` PROCEDURE `ShowMonthlyCost`(
)
BEGIN
...
DELIMITER //
DROP PROCEDURE IF EXISTS ShowYearlyRevenue
// DELIMITER;


DELIMITER //
CREATE DEFINER=`dev`@`localhost` PROCEDURE `ShowYearlyRevenue`(
)
BEGIN
  ...
  SELECT 
    ...
    FROM jobs 
    INNER JOIN jobstasks ON jobs.JobId = jobstasks.JobId
	WHERE YEAR(Start) = YEAR((SELECT NOW()));

END //
DELIMITER ;

DELIMITER //
DROP PROCEDURE IF EXISTS ShowYearlyCostToDate
// DELIMITER;


DELIMITER //
CREATE DEFINER=`dev`@`localhost` PROCEDURE `ShowYearlyCostToDate`(
)
BEGIN
...
DELIMITER //
DROP PROCEDURE IF EXISTS NumberOfJobsToday
// DELIMITER;

DELIMITER //
CREATE DEFINER=`dev`@`localhost` PROCEDURE `NumberOfJobsToday`(
)
BEGIN

  SELECT COUNT(*) FROM jobs
WHERE date(Start) = DATE((SELECT NOW()));

END //
DELIMITER ;

DELIMITER //
DROP PROCEDURE IF EXISTS CurrentTeamOnCall
// DELIMITER;

DELIMITER //
CREATE DEFINER=`dev`@`localhost` PROCEDURE `CurrentTeamOnCall`(
)
BEGIN
...
BEGIN
	IF (Select COUNT(teamid) from teams WHERE isDeleted = 0 AND isoncall = 1) = 0 THEN 
	update teams SET  IsOnCall = 1 where TeamId = TeamId_param;
    END IF;
...
INSERT INTO `teams`(`Name`, `IsOnCall`,`IsDeleted`,`CreatedAt`, `UpdatedAt`, `DeletedAt`)
							VALUES('Awesome', 0, 0, (SELECT NOW()), null, null),
                                  ...
                                  ('South', 0, 0,'2016-05-05 00:00', null, null),
								  ('North', 0, 0,'2016-05-05 00:00', null, null),
									('West', 0, 0,'2016-05-05 00:00', null, null),
                                    ('East', 0, 0,'2016-05-05 00:00', null, null),
                                     ('Benevolent', 0, 0,'2017-09-23 00:00', null, null),
								   ('Sporty', 0, 0,'2017-09-23 00:00', null, null),
									 ('Talky', 0, 0,'2018-09-23 00:00', null, null),
                                      ('Trashy', 0, 0,'2018-09-23 00:00', null, null),
                                     ('Lengthy', 0, 0,'2019-09-23 00:00', null, null),
                                      ('Salsa', 0, 0,'2019-05-05 00:00', null, null),
									('Hwasa', 0, 0,'2020-05-05 00:00', null, null),
                                  ('Sith', 0, 0, (SELECT NOW()), null, null);
 
INSERT INTO `jobs`(`TeamId`,`ClientId`, `Description`,`Start`,`End`,`OnSite`)
							VALUES
                            (1, 1, 'The job is something','2021-05-14 10:00', '2021-05-14 11:00', 0),
								  (2, 2, 'Doing thangs','2021-05-14 09:00', '2021-05-14 09:30', 0),
                                  (3, 3, 'Assembling company`s Intranet','2021-04-17 10:00', '2021-04-17 10:45', 0),
                                  (4, 4, 'Server System Implementation','2021-04-18 10:00', '2021-04-18 12:00', 1),
                                    (1, 1, 'The job is something','2021-03-14 10:00', '2021-03-14 11:00', 0),
								  (2, 2, 'Doing thangs','2021-03-14 09:00', '2021-05-14 09:30', 0),
                                  (3, 3, 'Assembling company`s Intranet','2021-02-17 10:00', '2021-02-15 10:45', 0),
                                   (1, 1, 'The job is something','2021-02-14 10:00', '2021-02-14 11:00', 0),
								  (2, 2, 'Doing thangs','2021-01-14 09:00', '2021-01-14 09:30', 0),
                                  (3, 3, 'Assembling company`s Intranet','2020-12-17 10:00', '2020-12-17 10:45', 0),
                                  (4, 4, 'Server System Implementation','2020-12-18 10:00', '2020-12-18 12:00', 1),
                                    (1, 1, 'The job is something','2020-11-14 10:00', '2020-11-14 11:00', 0),
								  (2, 2, 'Doing thangs','2020-11-10 09:00', '2020-11-10 09:30', 0),
                                  (3, 3, 'Assembling company`s Intranet','2020-10-17 10:00', '2020-10-17 10:45', 0),
                                      (3, 3, 'Assembling company`s Intranet','2020-09-20 10:00', '2020-09-20 10:45', 0),
                                  (3, 3, 'Assembling company`s Intranet','2020-09-17 10:00', '2020-09-17 10:45', 0),
                                  (4, 4, 'Server System Implementation','2020-08-18 10:00', '2020-08-18 12:00', 1),
                                    (1, 1, 'The job is something','2020-08-14 10:00', '2020-08-14 11:00', 0),
								  (2, 2, 'Doing thangs','2020-07-10 09:00', '2021-07-10 09:30', 0),
                                  (3, 3, 'Assembling company`s Intranet','2020-07-17 10:00', '2020-07-17 10:45', 0),
                                      (3, 3, 'Assembling company`s Intranet','2020-06-20 10:00', '2020-06-20 10:45', 0),
                                  (4, 4, 'Server System Implementation','2020-06-18 10:00', '2020-06-15 12:00', 1),
                                    (3, 3, 'Assembling company`s Intranet','2020-05-17 10:00', '2020-05-17 10:45', 0),
                                  (4, 4, 'Server System Implementation','2020-05-18 10:00', '2020-05-18 12:00', 1),
                                    (1, 1, 'The job is something','2020-04-14 10:00', '2020-04-14 11:00', 0),
								  (2, 2, 'Doing thangs','2020-04-10 09:00', '2020-04-10 09:30', 0),
                                  (3, 3, 'Assembling company`s Intranet','2020-03-17 10:00', '2020-03-17 10:45', 0),
                                      (3, 3, 'Assembling company`s Intranet','2020-03-20 10:00', '2020-03-20 10:45', 0),
                                  (3, 3, 'Assembling company`s Intranet','2020-02-17 10:00', '2020-02-17 10:45', 0),
                                  (4, 4, 'Server System Implementation','2020-02-18 10:00', '2020-02-18 12:00', 1),
                                    (1, 1, 'The job is something','2020-01-14 10:00', '2020-01-14 11:00', 0),
								  (2, 2, 'Doing thangs','2019-07-10 09:00', '2019-07-10 09:30', 0),
                                  (3, 3, 'Assembling company`s Intranet','2019-07-17 10:00', '2019-07-17 10:45', 0),
                                      (3, 3, 'Assembling company`s Intranet','2019-06-20 10:00', '2019-06-20 10:45', 0),
                                  (4, 4, 'Server System Implementation','2019-06-18 10:00', '2019-06-15 12:00', 1),
                                  (2, 2, 'Doing thangs','2018-07-10 09:00', '2018-07-10 09:30', 0),
                                  (3, 3, 'Assembling company`s Intranet','2018-07-17 10:00', '2018-07-17 10:45', 0),
                                      (3, 3, 'Assembling company`s Intranet','2018-06-20 10:00', '2018-06-20 10:45', 0),
                                  (4, 4, 'Server System Implementation','2018-06-18 10:00', '2018-06-15 12:00', 1),
                                    (2, 2, 'Doing thangs','2017-07-10 09:00', '2017-07-10 09:30', 0),
                                  (3, 3, 'Assembling company`s Intranet','2017-07-17 10:00', '2017-07-17 10:45', 0),
                                      (3, 3, 'Assembling company`s Intranet','2017-06-20 10:00', '2017-06-20 10:45', 0),
                                  (4, 4, 'Server System Implementation','2017-06-18 10:00', '2017-06-15 12:00', 1),
                                       (2, 2, 'Doing thangs','2016-07-10 09:00', '2016-07-10 09:30', 0),
                                  (3, 3, 'Assembling company`s Intranet','2016-07-17 10:00', '2016-07-17 10:45', 0),
                                      (3, 3, 'Assembling company`s Intranet','2016-06-20 10:00', '2016-06-20 10:45', 0),
                                  (4, 4, 'Server System Implementation','2016-06-18 10:00', '2016-06-15 12:00', 1),
                                         (2, 2, 'Doing thangs','2021-04-16 09:00', '2016-07-10 10:30', 0),
                                  (3, 3, 'Assembling company`s Intranet','2021-04-16 11:00', '2016-04-16 11:45', 0),
                                                                (3, 3, 'Assembling company`s Intranet','2021-04-16 13:00', '2016-04-16 13:45', 0),
                                  (4, 4, 'Server System Implementation','2021-04-16 10:00', '2021-04-16 12:00', 1);
